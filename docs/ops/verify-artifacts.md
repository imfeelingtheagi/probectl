...
Each release publishes `probectl_<tag>_sbom.spdx.json` — an SPDX JSON SBOM of
the source tree and every Go/npm dependency, generated by syft at release
time. It ships with its own `.sig`/`.pem` and verifies with the same
`cosign verify-blob` invocation as any binary above. Feed it to your
SCA/licensing tooling directly.
