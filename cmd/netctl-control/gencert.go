...
//
//	netctl-control gen-cert [dir]      # default dir: the current directory
//	NETCTL_CERT_HOSTS=host1,host2 ...  # SANs (default: localhost,127.0.0.1)
func genCert(args []string) error {
	...
	if env := strings.TrimSpace(os.Getenv("NETCTL_CERT_HOSTS")); env != "" {
		hosts = splitTrim(env)
	}

	const ttl = 365 * 24 * time.Hour
	ca, err := crypto.GenerateCA("netctl quickstart CA", ttl)
	if err != nil {
		return fmt.Errorf("generate CA: %w", err)
	}
	certPEM, keyPEM, err := ca.IssueServerCert("netctl", hosts, ttl)
	if err != nil {
