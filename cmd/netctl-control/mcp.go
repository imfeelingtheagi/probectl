...
// Claude Desktop). The token comes from NETCTL_MCP_TOKEN. The caller has already
...
func runMCPStdio(cfg *config.Config, log *slog.Logger, db *store.DB) error {
	token := os.Getenv("NETCTL_MCP_TOKEN")
	if token == "" {
		return fmt.Errorf("NETCTL_MCP_TOKEN is required for mcp-stdio")
	}
